    ...
    try {
      ...
      if (requiredPermissionName === "eliminar") {
        return res.status(403).json({
          success: false,
          message:
            "Acceso denegado: Solo el personal administrativo puede eliminar registros permanentes.",
        });
      }
      ...
      if (!municipioId) {
        return res.status(400).json({
          success: false,
          message:
            "Identificador de municipio no proporcionado para validar privilegios territoriales.",
        });
      }
